...
const url = "mongodb://localhost:27017/xiaogao";

//连接数据库
function connect(backFn){
	MongoClient.connect(url, { useNewUrlParser: true },(err,db)=>{
		if(err) throw err;
		var dbase = db.db("xiaogao");
		backFn(dbase);
		db.close()
	})
}
...
exports.find = function(collectionName,query,callback){
	...
	connect(function(dbase){
		dbase.collection(collectionName).find(query.where).skip(query.skip).limit(query.limit).toArray(function(err,data){
			callback(err,data)
		})
	})
}
